...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_BALANCELOG: 'balanceLog/PARTIAL_UPDATE_BALANCELOG',
...
export default (state: BalanceLogState = initialState, action): BalanceLogState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_BALANCELOG):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_BALANCELOG):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_BALANCELOG):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const getEntities: ICrudGetAllAction<IBalanceLog> = (page, size, sort) => ({
  ...
  payload: axios.get<IBalanceLog>(`${apiUrl}?cacheBuster=${new Date().getTime()}`),
});
...
export const partialUpdate: ICrudPutAction<IBalanceLog> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_BALANCELOG,
