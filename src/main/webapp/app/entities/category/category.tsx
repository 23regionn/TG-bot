          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>Boolean 1</th>
                <th>Chanell Id</th>
                <th>Links By Category In Top Id</th>
                <th />
              </tr>
            </thead>
            <tbody>
              {categoryList.map((category, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{category.name}</td>
                  <td>{category.countChanellInCategory}</td>
                  <td>{category.isDelete ? 'true' : 'false'}</td>
                  <td>{category.date1 ? <TextFormat type="date" value={category.date1} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{category.date2 ? <TextFormat type="date" value={category.date2} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{category.long1}</td>
                  <td>{category.string1}</td>
                  <td>{category.boolean1 ? 'true' : 'false'}</td>
                  <td>
                    {category.chanellIds
                      ? category.chanellIds.map((val, j) => (
                          <span key={j}>
                            <Link to={`chanell/${val.id}`}>{val.id}</Link>
                            {j === category.chanellIds.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>
                    {category.linksByCategoryInTopIds
                      ? category.linksByCategoryInTopIds.map((val, j) => (
                          <span key={j}>
                            <Link to={`links-by-category-in-top/${val.id}`}>{val.id}</Link>
                            {j === category.linksByCategoryInTopIds.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
