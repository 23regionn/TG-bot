...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_CITY: 'city/PARTIAL_UPDATE_CITY',
...
export default (state: CityState = initialState, action): CityState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_CITY):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_CITY):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_CITY):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<ICity> = entity => async dispatch => {
  const result = await dispatch({
