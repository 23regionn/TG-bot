    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            /*else if(messageText.equals("Текстовый поиск категорий 🌍🌍🌍")){
                ...
                mark.getKeyboard().get(0).get(0).setCallbackData(
                    update.getMessage().getReplyMarkup().getKeyboard().get(0).get(0).getCallbackData()+chatId
                );
...
//                sendMessage(chatId, textToSend, nameForLog);
        ...
        // Обработка запроса и формирование ответа
            ...
            var categories = categoryRepository.findCategoriesHaveChanellsAndIsShowTrue(ZonedDateTime.now().minusDays(1)).stream()
            ...
            for (int i = 0; i < categories.size(); i++) {
                ...
                results.add(new InlineQueryResultArticle(("" + i),  name,
                    messageContent, markupInLine, null, null, null, null, null, null));}
...
//                addChannelName(chatId, nameForLog, idChannel, tgUserId);
                ...
                switch (String.valueOf(callbackData.split(":")[2])) {
                    ...
                    case "5": whyFailureINT = (" не верно указана цена "); break;
                    case "6": whyFailureINT = (" канал относится к запретным "); break;
                    case "8": whyFailureINT = (" не корректное описание "); break;
                    default: whyFailureINT = (""); break;
                }
...
//         message.setText("Выберите  \"действие\" "); // Обязательное для телеграмма-апи поле
    ...
    private void findCategoryFirstPage(long chatId, String name, boolean isEditMessage, long messageId){
        ...
        List<CategoryNameAndIdDTO> categoryListFromDB = categoryRepository.findCategoriesHaveChanellsAndIsShowTrue(ZonedDateTime.now().minusDays(1)); //categoryListFirstPage
        List<CategoryNameAndIdDTO>  categoryListFirstPage = categoryListFromDB.stream().filter(CategoryNameAndIdDTO::getFirst)
            .sorted(Comparator.comparing(CategoryNameAndIdDTO::getScore)).collect(Collectors.toList());

        List<CategoryNameAndIdDTO>  categoryListNotFirst = categoryRepository.findCategoriesHaveChanellsAndIsShowTrue(ZonedDateTime.now().minusDays(1)).stream()
            .filter(cat -> cat.getFirst().equals(false))
            .sorted(Comparator.comparing(CategoryNameAndIdDTO::getScore)).collect(Collectors.toList());

        Map<Integer, List<CategoryNameAndIdDTO>> groupedCategories = IntStream.range(0, categoryListNotFirst.size())
            .boxed()
            .collect(Collectors.groupingBy(i -> i / 10,
                Collectors.mapping(categoryListNotFirst::get, Collectors.toList())));
    ...
    private void findCategoryNextPage(long chatId, String name, Integer pageNumber, long messageId, Integer numberInMap){
        ...
        List<CategoryNameAndIdDTO> categoryListFromDB = categoryRepository.findCategoriesHaveChanellsAndIsShowTrue(ZonedDateTime.now().minusDays(1));

        List<CategoryNameAndIdDTO>  categoryListFirstPage = categoryListFromDB.stream().filter(CategoryNameAndIdDTO::getFirst)
            .sorted(Comparator.comparing(CategoryNameAndIdDTO::getScore)).collect(Collectors.toList());

        List<CategoryNameAndIdDTO>  categoryListNotFirst = categoryListFromDB.stream()
            .filter(cat -> cat.getFirst().equals(false))
            .sorted(Comparator.comparing(CategoryNameAndIdDTO::getScore)).collect(Collectors.toList());
...
//        String nameCity = cityRepository.findById(cityId).get().getCityName();
        ...
        List<CategoryNameAndIdDTO> categoryListFromDB = categoryRepository
            .findCategoriesByCityId(cityId, ZonedDateTime.now().minusDays(1));
        ...
        List<CategoryNameAndIdDTO>  categoryNotFirst = categoryListFromDB.stream()
            .filter(cat -> cat.getFirst().equals(false))
            .sorted(Comparator.comparing(CategoryNameAndIdDTO::getScore)).collect(Collectors.toList());

        Map<Integer, List<CategoryNameAndIdDTO>> groupedCategories = IntStream.range(0, categoryNotFirst.size())
            .boxed()
            .collect(Collectors.groupingBy(i -> i / 10,
                Collectors.mapping(categoryNotFirst::get, Collectors.toList())));
...
//            Map<Integer, List<String>> mapInfoChannels = new LinkedHashMap<>();
            ...
            Map<Integer, List<String>> map = IntStream.range(0, allChanellsInformationList.size())
                .boxed()
                .collect(Collectors.groupingBy(i -> i / 3,
                    Collectors.mapping(allChanellsInformationList::get, Collectors.toList())));
...
//            allChanellsInformationList.stream().limit(3).forEach(str-> stringBuilder.append(str));
            ...
            message.setText("Категория: " + category.get().getName() + "\n" +
                            "Каналов в категории: " + chanellList.size() + "\n" +
                            "Всего страниц с каналами: " + map.size() + "\n" +
                            "Текущая страница: 1" +  "\n" +
                            "Выберите каналы: \n " + stringBuilder +
                    " \n\nЕщё каналы ниже⬇⬇⬇");
    ...
    private void getChanellByCategoryIdAndPageNumber(long chatId, String name, Long categoryId, Long pageNumberInMap, long messageId){
        ...
        if (chanellList.isEmpty()){
            ...
            message.setText("Категория: " + category.get().getName() + "\n" +
                "Каналов в категории: " + chanellList.size() + "\n" +
                "Всего страниц с каналами: " + map.size() + "\n" +
                "Текущая страница: " + (pageNumberInMap + 1) + "\n" +
                "Выберите каналы: \n " + stringBuilder +
                " \n\nЕщё каналы ниже⬇⬇⬇");
...
//        Optional<Category> category = categoryRepository.findOneWithEagerRelationships(categoryId);
        CategoryWithCountChanellsDTO category = categoryRepository.findCategoryByIdWithoutCity(categoryId, ZonedDateTime.now().minusDays(1));
            message.setText("Категория: " + category.getName().toUpperCase() + "\nКаналов в категории: " + category.getCountChannelsInCategory() +
...
//        Optional<Category> category = categoryRepository.findOneWithEagerRelationships(categoryId, cityName);
        Optional<Category> category = categoryRepository.findOneWithEagerRelationshipsWithCityId(categoryId, cityId, ZonedDateTime.now().minusDays(1));
        List<Chanell> chanellList = new ArrayList<>();
        if (category.isPresent()) {
    ...
    private void getChanellByCityNameByCategoryId(long chatId, String name, Long cityId, Long categoryId){
        ...
        if (chanellList.isEmpty()){
            ...
            map.get(0).forEach(str-> stringBuilder.append(str));
            message.setText("Город: " + nameCity + "\n" + "Категория: " + category.getName() + "\n" +
                "Каналов в категории: " + chanellList.size() + "\n" +
                "Всего страниц с каналами: " + map.size() + "\n" +
                "Текущая страница: 1" +  "\n" +
                "Выберите каналы: \n " + stringBuilder +
...
//                rowsInLine.add(rowInLine);
    ...
    private void getChanellByCityNameByCategoryIdAndPageNumber(long chatId, String name, Long cityId,
        ...
        List<Chanell> chanellList = chanellRepository.findChannelsByCityIDAndCategory(cityId, categoryId, ZonedDateTime.now().minusDays(1))
            .stream().filter(c -> c.getScore() != null)
            .sorted(Comparator.comparing(Chanell::getScore))
            .collect(Collectors.toList());
    ...
    private void getCategoriesByCityNameFirstPage(long chatId, String name, Long cityId, boolean isEditMessage, long messageId){
        ...
        // ВСТАВИЛ НА СЛУЧАЙ, КОГДА НЕТ ФЁРСТ КАТЕГОРИЙ
        List<CategoryNameAndIdDTO>  categoryNotFist = categoryListFromDB.stream()
            .filter(cat -> cat.getFirst().equals(false))
            .sorted(Comparator.comparing(CategoryNameAndIdDTO::getScore)).collect(Collectors.toList());

        Map<Integer, List<CategoryNameAndIdDTO>> groupedCategories = IntStream.range(0, categoryNotFist.size())
            .boxed()
            .collect(Collectors.groupingBy(i -> i / 10,
                Collectors.mapping(categoryNotFist::get, Collectors.toList())));
...
//            TgUserRepository.save(user);
   ...
   /* // отправить ответное сообщение с базовой  клавиатурой
    ...
    private void executeEditTextWithKeyBoardAndDisableWebPreview(long chatId, String nameForLog,
                                                                 String textMessage, long messageId,
    ...
    private void sendEditChannelToModerate(long adminId, long editChannelId){
        ...
        String msText =  new String
            ( "МОДЕРАЦИЯ КАНАЛА: \n" + "Пользователь: @" + chanell.getTGUser().getFirstName().toString() + "\n" +
                "ID: " + chanell.getTGUser().getChatId().toString() + "\n" +
                "Текущее Имя канала: " + chanell.getName().toString() + "\n" +
                "Пользователь хочет изменить имя на " + editChannels.getNewNameChannel() + "\n" +
                "Ссылка: " + chanell.getLink() + "\n" +
